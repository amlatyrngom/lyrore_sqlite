...
Expr* LyExpr::to_sqlite_new(Parse* pParse) const {
    ...
    switch (tk_op) {
        case TK_COLUMN:
            ...
            if (sqlite_expr) {
                pNew = sqlite3ExprDup(db, sqlite_expr, 0);
            } else {
                // Fallback: create column expr without binding (may fail)
                pNew = sqlite3Expr(db, TK_COLUMN, column_name.c_str());
                if (pNew) {
        ...
        case TK_INTEGER:
            if (std::holds_alternative<int64_t>(value)) {
                ...
                pNew = sqlite3Expr(db, TK_INTEGER, nullptr);
                if (pNew) {
                    pNew->flags |= EP_IntValue;
                    pNew->u.iValue = static_cast<int>(v);
                }
            } else {
                pNew = sqlite3Expr(db, TK_INTEGER, token.c_str());
            }
            break;
            
        case TK_FLOAT:
            pNew = sqlite3Expr(db, TK_FLOAT, token.c_str());
            break;
            
        case TK_STRING:
            if (std::holds_alternative<std::string>(value)) {
                pNew = sqlite3Expr(db, TK_STRING, std::get<std::string>(value).c_str());
            } else {
                pNew = sqlite3Expr(db, TK_STRING, token.c_str());
            }
            break;
            
        case TK_NULL:
            pNew = sqlite3Expr(db, TK_NULL, nullptr);
            break;
        ...
        case TK_AGG_FUNCTION:
            {
                ...
                pNew = sqlite3Expr(db, tk_op, token.c_str());
                if (!pNew) break;
                ...
                if (!args.empty()) {
                    ...
                    for (const auto& arg : args) {
                        ...
                        pList = sqlite3ExprListAppend(pParse, pList, pArgExpr);
                    }
                    pNew->x.pList = pList;
                }
            }
        ...
        default:
            ...
            pNew = sqlite3Expr(db, tk_op, nullptr);
            break;
    }
...
void LyExpr::to_sqlite(Parse* pParse, Expr* pTarget) const {
    ...
    sqlite3ExprDelete(db, pTarget->pLeft);
    sqlite3ExprDelete(db, pTarget->pRight);
    if (ExprUseXList(pTarget) && pTarget->x.pList) {
        sqlite3ExprListDelete(db, pTarget->x.pList);
    }
